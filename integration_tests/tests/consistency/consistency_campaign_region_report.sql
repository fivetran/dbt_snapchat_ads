...
with prod as (
    ...
    from {{ target.schema }}_snapchat_ads_prod.snapchat_ads__campaign_region_report
    group by 1,2
),
...
dev as (
    select
        ...
        sum(total_conversions) as total_conversions,
        sum(conversion_purchases_value) as conversion_purchases_value
    from {{ target.schema }}_snapchat_ads_dev.snapchat_ads__campaign_region_report
    group by 1,2
),
